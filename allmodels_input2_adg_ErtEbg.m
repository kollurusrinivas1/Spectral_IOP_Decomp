function [stats_allmodels,stats_allmodels2,temp_all_adg] = allmodels_input2_adg_ErtEbg(models,data,subcomp,lambda_idx)
...
stats_rensemble_ds_adg412_bag_valente = stats_new(subcomp(:,lambda_idx),temp_all_adg(1,:)');
...
stats_ert = stats_new(subcomp(:,lambda_idx),temp_all_adg(2,:)');
fprintf('Extremely randomized trees model completed\n');


stats_allmodels.reg_ensem_bag = stats_rensemble_ds_adg412_bag_valente;
stats_allmodels.ert = stats_ert;

stats_allmodels2(1,:) = cell2mat(struct2cell(stats_rensemble_ds_adg412_bag_valente(1,:)));
